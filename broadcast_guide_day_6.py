...
DEFAULT_RUN_AT = datetime(2026, 6, 30, 12, 19, tzinfo=MOSCOW_TZ)
BROADCAST_SENT_EVENT = "mini_guide_day_6_sent_2026_06_30_1219"
USER_SEND_DELAY_SECONDS = 0.05

BROADCAST_TEXT = """6️⃣ <b>Где граница вашей компетенции</b>

Вы в рамках психологической компетенции, когда:
🔹 исследуете убеждения, эмоции и поведение вокруг еды и тела;
🔹 работаете с перфекционизмом, эмоциональной регуляцией и навыками совладания;
🔹 используете мягкий скрининг (вопросы + при необходимости опросники вроде SCOFF) и фиксируете риски.

При подозрении на клинически значимое РПП ваша задача — не задерживать направление к врачу/ психиатру и при возможности координировать работу с мультидисциплинарной командой."""
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description="Send mini-guide day 6 broadcast to all RPP bot users."
    )
    parser.add_argument(
        "--now",
        action="store_true",
        help="Send immediately instead of waiting until 2026-06-30 12:19 MSK.",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
