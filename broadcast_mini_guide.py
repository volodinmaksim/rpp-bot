...
FIRST_MESSAGE = """Привет. Мы подготовили для вас мини-гайд <b>“Как говорить с клиентом о еде и весе, чтобы помочь, а не навредить”</b>.

Расстройства пищевого поведения (РПП) широко распространены, но часто протекают скрыто и не зависят от веса или формы тела.

Задача терапевта — экологично исследовать мысли, эмоции, поведение вокруг еды, тела и вовремя увидеть ситуации, которые требуют медицинской помощи.

<b>У нас есть 8 коротких советов, которые мы будем присылать раз в день. И первый уже сегодня 🖐</b>"""

SECOND_MESSAGE = """1️⃣ <b>Как начать разговор</b>

Лучше не начинать с цифр веса или диагнозов. Сначала рекомендуем запросить согласие и связать тему с заботой о клиенте.

<b>Примеры:</b>
🔹 «Чтобы лучше понять, что с вами происходит, хочу задать несколько вопросов о питании, сне и энергии. Насколько вам комфортно обсудить это?»
🔹 «Многие люди замечают, что стресс влияет на аппетит и отношение к телу. Как это у вас?»

Это снижает защиту, нормализует опыт и усиливает ощущение безопасности."""

SECOND_MESSAGE_DELAY_SECONDS = 2 * 60
USER_START_DELAY_SECONDS = 0.1
BROADCAST_SENT_EVENT = "mini_guide_broadcast_sent_2026_06_25"
...
async def send_message_safely(user_id: int, text: str) -> bool:
    try:
        ...
        return await send_message_safely(user_id, text)
    except (TelegramForbiddenError, TelegramBadRequest) as exc:
        logger.warning("Skipped user %s: %s", user_id, exc)
        return False
    except Exception:
...
async def send_user_chain(user_id: int, *, mark_sent_event: bool) -> tuple[bool, bool]:
    ...
    second_sent = await send_message_safely(user_id, SECOND_MESSAGE)
...
async def send_broadcast(user_ids: list[int], *, dry_run: bool) -> None:
    ...
    logger.info(
        "Mini-guide broadcast stats: first=%s/%s, second=%s/%s",
        first_sent_count,
        len(user_ids),
        second_sent_count,
        len(user_ids),
...
async def run_broadcast(*, dry_run: bool, include_already_sent: bool) -> None:
    ...
    logger.info("Starting mini-guide broadcast for %s users", len(user_ids))
    await send_broadcast(user_ids, dry_run=dry_run)
    logger.info("Mini-guide broadcast finished")


async def main() -> None:
    parser = argparse.ArgumentParser(
        description="Send mini-guide broadcast chain to RPP bot users."
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
