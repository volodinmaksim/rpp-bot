...
DEFAULT_RUN_AT = datetime(2026, 6, 27, 12, 14, tzinfo=MOSCOW_TZ)
BROADCAST_SENT_EVENT = "mini_guide_day_3_sent_2026_06_27_1214"
USER_SEND_DELAY_SECONDS = 0.05

BROADCAST_TEXT = """3️⃣ <b>Переедание и еда как регуляция эмоций</b>
Клиенты часто живут в цикле «ограничение — срыв — вина» и считают это слабой волей. Для оценки важны и объём, и ощущение потери контроля.

<b>Спросите:</b>
🔹 «Бывают ли эпизоды, когда вы едите больше, чем планировали, и кажется, что сложно остановиться?»
🔹 «Как вы себя чувствуете после таких эпизодов? Появляются ли стыд, вина, самокритика?»
🔹 «Используете ли вы еду, чтобы справиться со стрессом, тревогой, пустотой или скукой?»

Можно ориентироваться на простые скрининговые вопросы, например из SCOFF, помните, что 2 и более «да» — повод уточнить детали."""
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description="Send mini-guide day 3 broadcast to all RPP bot users."
    )
    parser.add_argument(
        "--now",
        action="store_true",
        help="Send immediately instead of waiting until 2026-06-27 12:14 MSK.",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
